...
| `morning_scan_jumpday_long.pine` | Long-only rider (Pine **v6**) | **For hyper-volatile movers (avg range ≥ ~7%/day: HIMS/QBTS/IONQ class.)** Trades ONLY scan-qualified **gap-up** days, one morning 9-EMA pullback entry above VWAP, structural stop, ride to the 15:50 flatten. Survivor of a 19-config sweep + holdouts (IONQ unseen: +$2,413 PF 1.53). Edge decays below ~6%/day range — see `research/findings.md` case study 3 |
| `morning_scan_largecap_scalper.pine` | Long-only scalper (Pine **v6**) | **For liquid large caps (≥ ~$10B/day dollar volume: COIN/TSLA/NVDA/AMD/META class).** Same gap-up day filter + pullback entry, but takes 1.5R profit targets, up to 3 trades/day, pre-market volume measured in dollars. Validated frozen on 3 unseen symbols, all positive (AMD +$1,364 PF 1.47). Cheap <$20 movers (PLUG/BBAI class): NO long edge found — don't trade mechanically |
